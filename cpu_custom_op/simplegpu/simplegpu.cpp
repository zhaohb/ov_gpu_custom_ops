...
//! [op:ctor]
SimpleGPU::SimpleGPU(const ov::Output<ov::Node>& A, const ov::Output<ov::Node>& B, const ov::Output<ov::Node>& C , int32_t queryLen, int32_t kvSeqLen, int32_t batchSize, int32_t hasMask): Op({A, B, C}), queryLen(queryLen), kvSeqLen(kvSeqLen), batchSize(batchSize), hasMask(hasMask){
...
std::shared_ptr<ov::Node> SimpleGPU::clone_with_new_inputs(const ov::OutputVector& new_args) const {
    ...
    return std::make_shared<SimpleGPU>(new_args[0], new_args[1], new_args[2], queryLen, kvSeqLen, batchSize, hasMask);
}
//! [op:copy]
